...
class CNNModel(AbstractModel):
    ...
    def get_prediction(self, x_values, y_values, train_end_index, consumer_index):
        ...
        if self.model is None:
            model_file = os.path.join('trained_models/', 'model_cnn_series_{}.h5'.format(consumer_index))
            if os.path.exists(model_file):
                ...
                self.model = self.cnn_model(num_days=self.num_days2see,
    ...
    @staticmethod
    def cnn_model(feat_length=26, num_days=30):
        model = Sequential()
        model.add(Conv2D(36, 3, activation='relu', input_shape=(48 * num_days, feat_length, 1)))
