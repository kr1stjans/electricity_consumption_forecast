...
from test.data_manager import DataManager
import pandas as pd
...
# def build_tsfresh(data):
# remove columns from TSFRESH
#    data = pd.DataFrame(
#        {'id': ['A'] * len(data[:96]), 'dt': data[:96, 1], 'x': np.array(data[:96, 0], dtype=np.float64)})
#    extracted_features = extract_features(data, column_id="id", column_sort="dt")
